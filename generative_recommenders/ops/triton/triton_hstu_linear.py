...
def triton_layer_norm_mul_dropout_fwd(
    ...
    if (
        ...
        and is_sm100_plus()
        and (not concat_u and not concat_x)
        and training
    ):
...
def _swiglu_tma_set_block_size_hook(nargs) -> None:
    ...
    # Weights are in [N, K] layout (nn.Linear convention)
...
def get_swiglu_configs(pre_hook=None) -> List[triton.Config]:
    return [
        triton.Config(
            {
                "BLOCK_M": 128,
...
def _swiglu_fwd_tma_ws_persistent(
    ...
    # pyre-ignore[6]
    smem_empty_bars = tlx.alloc_barriers(num_barriers=NUM_SMEM_BUFFERS, arrive_count=2)
        ...
        with tlx.async_task(num_warps=1, num_regs=24):
            ...
            for tile_id in range(start_pid, num_tiles, NUM_SMS):
                ...
                for k in range(0, k_tiles):
                    ...
                    if processed_k_iters + k >= NUM_SMEM_BUFFERS:
                    ...
                    tlx.barrier_expect_bytes(
                        smem_full_bars[buf],
                        # pyre-ignore[6]
                        2 * (BLOCK_M * BLOCK_K + 2 * BLOCK_K * BLOCK_N),
                    )

                    # Perform TMA loads
                    tlx.async_descriptor_load(
                        x_desc, x_buffers[buf], [offs_m, offs_k], smem_full_bars[buf]
                    )
