...
@triton.jit
def _helion_split_2d_jagged_fp32_kernel(
    offsets_a,
    offsets_b,
    values_flat,
    out_a_flat,
    ...
    if has_work:
        ...
        # Process features in smaller tiles (64 at a time for FP32)
...
@triton.jit
def _helion_split_2d_jagged_bf16_kernel(
    offsets_a,
    offsets_b,
    values_flat,
    out_a_flat,
    ...
    out_a_end = tl.load(
        offsets_a + batch_id_plus_1 * 1, None, eviction_policy="evict_first"
    )
    len_a = out_a_end - out_a_start
    # Load output boundaries for part B
    out_b_start = tl.load(offsets_b + batch_id * 1, None)
    out_b_end = tl.load(offsets_b + batch_id_plus_1 * 1, None)
    len_b = out_b_end - out_b_start
    # Compute input start and total length for this batch
    input_start = out_a_start + out_b_start
    total_len = len_a + len_b
    ...
    if has_work:
        ...
        # Process entire feature dimension in one pass
        for feature_offset in tl.range(
            0, D, _BLOCK_SIZE_1, num_stages=3, disallow_acc_multi_buffer=True
        ):
            feature_indices = feature_offset + tl.arange(0, _BLOCK_SIZE_1).to(tl.int32)
            ...
            # Load and store part B data
            slice_b = tl.load(values_flat + input_idx_b * 1, mask_b, other=0)
            tl.store(out_b_flat + out_b_idx * 1, slice_b, mask_b)
...
def helion_split_2D_jagged(
    ...
    if values.dtype == torch.float32:
        return _helion_split_2d_jagged_fp32(
            values, max_seq_len, offsets_a, offsets_b, dense_size
        )
    else:
        return _helion_split_2d_jagged_bf16(
            values, max_seq_len, offsets_a, offsets_b, dense_size
        )


def _helion_split_2d_jagged_fp32(
    values: torch.Tensor,
    max_seq_len: int,
    offsets_a: torch.Tensor,
    offsets_b: torch.Tensor,
    ...
    """FP32-optimized wrapper"""
    num_batches = offsets_a.size(0) - 1
    D = values.size(1)
    BLOCK_N = 64
    num_seq_blocks = (max_seq_len + BLOCK_N - 1) // BLOCK_N
    total_len_a = offsets_a[-1].item()
    total_len_b = offsets_b[-1].item()
    ...
    _helion_split_2d_jagged_fp32_kernel[(total_programs,)](
        offsets_a,
        offsets_b,
        values_flat,
        out_a_flat,
...
def _helion_split_2d_jagged_bf16(
    values: torch.Tensor,
    max_seq_len: int,
    offsets_a: torch.Tensor,
    offsets_b: torch.Tensor,
    ...
    total_programs = num_batches * num_seq_blocks
    _BLOCK_SIZE_0 = 128
    _BLOCK_SIZE_1 = triton.next_power_of_2(D)

    _helion_split_2d_jagged_bf16_kernel[(total_programs,)](
        offsets_a,
        offsets_b,
        values_flat,
        out_a_flat,
