...
def _weighted_layer_norm_bwd_dx(
    ...
    for idx in range(blocks_per_tile):
        ...
        if IS_SWISH:
            ...
            x_weighted = dy_block * x_block * w[None, :] * sigmoid_deriv * rstd
            x_weighted = tl.where(
                row_mask[:, None] & col_mask[None, :], x_weighted, 0.0
            )

            c1 = tl.sum(xhat * x_weighted, axis=1) / D
            c2 = tl.sum(x_weighted, axis=1) / D
            c1 = tl.expand_dims(c1, 1)
            c2 = tl.expand_dims(c2, 1)
            dx_norm = (x_weighted - (xhat * c1 + c2)) * rstd

            dx = dy_block * sigmoid_layer_norm + dx_norm
        else:
            wdy = w[None, :] * dy_block
